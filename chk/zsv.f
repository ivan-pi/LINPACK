...
C
      ...
      COMPLEX*16 X(25,25),XX(25,25),U(25,25),V(25,25),S(25),E(25),
     *           WORK(25)
...
C
   ...
   30       CONTINUE
            ...
            X(3,3) = (3.0D0,3.0D0)
            X(3,4) = (1.0D0,1.0D0)
            CALL ZSVT1(X,LDX,N,P,S,E,U,LDU,V,LDV,WORK,XX,CASE,NOTWRT,
     *                 LUNIT,11)
         GO TO 280
C
...
C
   ...
   60       CONTINUE
            ...
            X(4,5) = (1.0D0,1.0D0)
            X(5,5) = (4.0D0,4.0D0)
            CALL ZSVT1(X,LDX,N,P,S,E,U,LDU,X,LDX,WORK,XX,CASE,NOTWRT,
     *                 LUNIT,11)
         GO TO 280
C
...
C
   70    CONTINUE
            ...
            P = 4
            DO 80 I = 1, 4
               S(I) = DCMPLX(DBLE(FLOAT(I)),0.0D0)
   80       CONTINUE
            CALL ZXGEN(X,LDX,N,P,S)
            CALL ZSVT1(X,LDX,N,P,S,E,X,LDX,V,LDV,WORK,XX,CASE,NOTWRT,
     *                 LUNIT,21)
         GO TO 280
C
...
C
  ...
  430 FORMAT (22H1LINPACK TESTER, ZSV** /
...
C
   ...
   70 FORMAT (1H , 8D13.6)
      ...
      COMPLEX*16 XX(LDX,1),S(1),U(LDU,1),V(LDV,1),X(LDX,1)
      DOUBLE PRECISION XSTAT
C
C     EXTERNAL ZMACH
C     FORTRAN DMAX1,CDABS,DCONJG,MIN0
C
      INTEGER I,J,K,M
      COMPLEX*16 T(25)
...
C
   ...
   90 CONTINUE
      ...
      DO 110 J = 1, P
         DO 100 I = 1, N
            EMAX = DMAX1(EMAX,CDABS(X(I,J)-XX(I,J)))
  ...
  140 CONTINUE
      RETURN
      END
      SUBROUTINE ZSVOT(X,LDX,N,COL,TEST)
      INTEGER LDX,N,COL
      COMPLEX*16 X(LDX,1)
C
C     FORTRAN DMAX1
C
      INTEGER I,J,K
      COMPLEX*16 ELM
      DOUBLE PRECISION TEST,EMAX,ZMACH
      DOUBLE PRECISION DREAL,DIMAG
      COMPLEX*16 ZDUMR,ZDUMI
      DREAL(ZDUMR) = ZDUMR
      DIMAG(ZDUMI) = (0.0D0,-1.0D0)*ZDUMI
      ...
      DO 30 I = 1, COL
         DO 20 J = 1, COL
            ELM = (0.0D0,0.0D0)
            DO 10 K = 1, N
               ELM = ELM + DCONJG(X(K,I))*X(K,J)
   10       CONTINUE
            IF (I .EQ. J) ELM = (1.0D0,0.0D0) - ELM
            EMAX = DMAX1(EMAX,CDABS(ELM))
   20    CONTINUE
   30 CONTINUE
      TEST = EMAX/ZMACH(1)
      RETURN
      END
      SUBROUTINE ZSVT1(X,LDX,N,P,S,E,U,LDU,V,LDV,WORK,XX,CASE,NOTWRT,
     *                 LUNIT,JOB)
      INTEGER LDX,N,P,LDU,LDV,CASE,LUNIT,JOB
      COMPLEX*16 X(LDX,1),S(1),E(1),U(LDU,1),V(LDV,1),WORK(1),XX(LDX,1)
      LOGICAL NOTWRT
C
C     EXTERNAL ZARRAY,ZSVBM,ZSVDC,ZSVOT
C     FORTRAN DMAX1,MIN0
C
      INTEGER I,J,M,UCOL,INFO
      DOUBLE PRECISION USTAT,VSTAT,XSTAT
      COMPLEX*16 XNEW(25,25)
   ...
   20 CONTINUE
      IF (NOTWRT) GO TO 30
         WRITE (LUNIT,50)
         CALL ZARRAY(X,LDX,N,P,5,LUNIT)
   30 CONTINUE
      CALL ZSVDC(X,LDX,N,P,S,E,U,LDU,V,LDV,WORK,JOB,INFO)
      M = MIN0(N,P)
      IF (NOTWRT) GO TO 40
         WRITE (LUNIT,60)
         CALL ZARRAY(S,P,M,1,-5,LUNIT)
         WRITE (LUNIT,70)
         CALL ZARRAY(U,LDU,N,N,5,LUNIT)
         WRITE (LUNIT,80)
         CALL ZARRAY(V,LDV,P,P,5,LUNIT)
   40 CONTINUE
      CALL ZSVOT(U,LDU,N,UCOL,USTAT)
      CALL ZSVOT(V,LDV,P,P,VSTAT)
...
C
C     FORTRAN DCMPLX,DBLE,FLOAT,MIN0
C
      INTEGER I,J,M,MP1
      COMPLEX*16 T,RU,FAC
