...
C
      ...
      STOP
      END
      SUBROUTINE CHITS(LUNIT)
C     LUNIT IS THE OUTPUT UNIT NUMBER
C
C     TESTS
C        CHICO,CHIFA,CHISL,CHIDI,CHPCO,CHPFA,CHPSL,CHPDI
C
...
C
C     LINPACK CHICO,CHISL,CHIDI,CHPCO,CHPSL,CHPDI
C     EXTERNAL CHIXX,CMACH
C     BLAS CAXPY,CSWAP,SCASUM,CCOPY
C     FORTRAN ABS,AIMAG,AMAX1,CONJG,FLOAT,IABS,REAL
C
C     INTERNAL VARIABLES
C
      COMPLEX A(15,15),AINV(15,15),ASAVE(15,15),AJK,AJKP1
      COMPLEX AP(120),B(15),X(15),XEXACT(15)
...
C
         RCOND = -1.0E0
         CALL CHICO(A,LDA,N,KPVT,RCOND,Z)
...
C
  ...
  130    CONTINUE
         RCONDP = -1.0E0
         CALL CHPCO(AP,N,KPVTP,RCONDP,Z)
...
C
         IF (RCOND .GT. 0.0E0) GO TO 180
            ...
            KSING = KSING + 1
            CALL CHIDI(A,LDA,N,KPVT,DET,INERT,Z,110)
...
C
  ...
  190          CONTINUE
  200       CONTINUE
            CALL CHIDI(AINV,LDA,N,KPVT,DET,INERT,Z,111)
            AINORM = 0.0E0
            DO 220 J = 1, N
               DO 210 I = J, N
...
C
  ...
  250       CONTINUE
            IF (KOUNT .EQ. 0) GO TO 260
               ...
               KPF = .TRUE.
  260       CONTINUE
            CALL CHPDI(AP,N,KPVTP,DETP,INERTP,Z,111)
...
C
                  ...
                  DO 350 JB = 1, KM1
                     J = K - JB
                     AJK = A(K,K)*A(J,K) + CONJG(A(K,K+1))*A(J,K+1)
...
C
      ...
      RETURN
C
C     MOST FORMATS, ALSO SOME IN CHIXX
C
  530 FORMAT (1H )
  540 FORMAT (29H1LINPACK TESTER, CHI**, CHP**)
  ...
  950 FORMAT (29H THIS VERSION DATED 08/14/78.)
      END
      SUBROUTINE CHIXX(A,LDA,N,KASE,LUNIT)
      INTEGER LDA,N,KASE,LUNIT
      COMPLEX A(LDA,1)
C
C     GENERATES COMPLEX HERMITIAN INDEFINITE TEST MATRICES
C
C     EXTERNAL CMACH
C     FORTRAN ABS,AIMAG,CABS,CMPLX,CONJG,FLOAT,IABS,REAL
      COMPLEX T
