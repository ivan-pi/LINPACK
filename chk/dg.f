...
C
      ...
      STOP
      END
      SUBROUTINE DGETS(LUNIT)
C     LUNIT IS THE OUTPUT UNIT NUMBER
C
C     TESTS
C        DGECO,DGEFA,DGESL,DGEDI,DGBCO,DGBFA,DGBSL,DGBDI
C
...
C
C     LINPACK DGECO,DGESL,DGEDI,DGBCO,DGBSL,DGBDI
C     EXTERNAL DGEXX,DMACH
C     BLAS DAXPY,DDOT,DSCAL,DASUM
C     FORTRAN DABS,DMAX1,DBLE,FLOAT,MAX0,MIN0
C
C     INTERNAL VARIABLES
C
      DOUBLE PRECISION A(15,15),AB(43,15),AINV(15,15),ASAVE(15,15)
      DOUBLE PRECISION B(15),BT(15),DDOT,DET(2),DETB(2)
      DOUBLE PRECISION X(15),XB(15),XEXACT(15),XT(15),XTB(15),T,Z(15)
      DOUBLE PRECISION AINORM,ANORM,DMACH,COND,COND1,EN,ENORM,EPS
      DOUBLE PRECISION ETNORM,FNI,FNORM,ONEPX,RCOND,RCONDB,RNORM
      DOUBLE PRECISION RTNORM,Q(8),QS(8),DASUM,XNORM,XTNORM
...
C
  ...
  170       CONTINUE
C
            CALL DGBCO(AB,LDAB,N,ML,MU,IPVTB,RCONDB,Z)
C
...
C
  ...
  220             CONTINUE
  230          CONTINUE
               CALL DGEDI(AINV,LDA,N,IPVT,DET,Z,11)
               AINORM = 0.0D0
               DO 240 J = 1, N
...
C
               CALL DGESL(A,LDA,N,IPVT,X,0)
               CALL DGESL(A,LDA,N,IPVT,XT,1)
C
...
C              MORE BAND COMPARE
C
               CALL DGBSL(AB,LDAB,N,ML,MU,IPVTB,XB,0)
               CALL DGBSL(AB,LDAB,N,ML,MU,IPVTB,XTB,1)
               KOUNT = 0
               DO 280 I = 1, N
  ...
  280          CONTINUE
               IF (KOUNT .EQ. 0) GO TO 290
                  ...
                  KBF = .TRUE.
  290          CONTINUE
               CALL DGBDI(AB,LDAB,N,ML,MU,IPVTB,DETB)
...
C
               ...
               DO 350 J = 1, N
                  ...
                  BT(J) = BT(J) - DDOT(N,ASAVE(1,J),1,XT,1)
                  FNI = 0.0D0
                  DO 340 I = 1, N
...
C
      ...
      RETURN
C
C     MOST FORMATS, ALSO SOME IN DGEXX
C
  450 FORMAT (1H )
  460 FORMAT (29H1LINPACK TESTER, DGE**, DGB**)
  ...
  880 FORMAT (29H THIS VERSION DATED 08/14/78.)
      END
      SUBROUTINE DGEXX(A,LDA,N,KASE,LUNIT)
