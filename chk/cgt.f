...
C
      INTEGER LUNIT
      COMPLEX B(20),BSAVE(20),D(20),EYE,C(20),E(20),X(20)
      REAL ANORM,EN,ENORM,EPS,Q(2),RNORM,SCASUM,XNORM
      REAL CMACH
      ...
      EYE = (0.0E0,1.0E0)
C     COMPLEX EYE = IMAGINARY UNIT, REAL EYE = ZERO
C
C
...
C
         CALL CGTXX(C,D,E,N,KASE,POSDEF)
C
...
C     ...EXIT
   ...
   30       CONTINUE
               NM1 = N - 1
               ANORM = CABS1(D(1)) + CABS1(C(2))
               IF (N .LE. 2) GO TO 50
                  DO 40 I = 2, NM1
                     ANORM = AMAX1(ANORM,
     *                             CABS1(C(I+1))+CABS1(D(I))
     *                             +CABS1(E(I-1)))
   40             CONTINUE
   50          CONTINUE
               ANORM = AMAX1(ANORM,CABS1(E(N-1))+CABS1(D(N)))
...
C
            IF (IPT .EQ. 1) CALL CGTSL(N,C,D,E,B,INFO)
C
...
C
               IF (IPT .EQ. 2) CALL CPTSL(N,D,E,B)
...
C
               ...
               CALL CGTXX(C,D,E,N,KASE,POSDEF)
               IF (N .GT. 1) GO TO 140
                  RNORM = CABS1(D(1)*B(1)-BSAVE(1))
                  ENORM = CABS1(B(1)-X(1))
               GO TO 170
  140          CONTINUE
                  ENORM = CABS1(B(1)-X(1))
                  RNORM = CABS1(D(1)*B(1)+E(1)*B(2)-BSAVE(1))
                  IF (N .LE. 2) GO TO 160
                     DO 150 I = 2, NM1
                        RNORM = RNORM
     *                          + CABS1(C(I)*B(I-1)+D(I)*B(I)
     *                                  +E(I)*B(I+1)-BSAVE(I))
                        ENORM = ENORM + CABS1(B(I)-X(I))
  150                CONTINUE
  160             CONTINUE
                  RNORM = RNORM + CABS1(C(N)*B(N-1)+D(N)*B(N)-BSAVE(N))
                  ENORM = ENORM + CABS1(B(N)-X(N))
  170          CONTINUE
C
...
C
  220 FORMAT (1H )
  230 FORMAT (29H1LINPACK TESTER, CGT**, CPT** /
  ...
  280 FORMAT (18H RESULTS FOR CGTSL)
  290 FORMAT (18H RESULTS FOR CPTSL)
  300 FORMAT ( / 4H X =)
  310 FORMAT ( / 4H B =)
  ...
  470 FORMAT ( / 12H END OF TEST)
      END
      SUBROUTINE CGTXX(C,D,E,N,KASE,POSDEF)
...
C
   30 CONTINUE
         N = (KASE - 2)*3
         DO 40 I = 1, N
            C(I) = (1.0E0,0.0E0)/(CMPLX(FLOAT(2*I+2),0.0E0) + EYE)
            D(I) = (1.0E0,0.0E0)/(CMPLX(FLOAT(2*I+1),0.0E0) + EYE)
            E(I) = (1.0E0,0.0E0)/(CMPLX(FLOAT(2*I),0.0E0) + EYE)
   40    CONTINUE
         POSDEF = 0
      GO TO 120
C
