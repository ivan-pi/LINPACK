...
C
C     LINPACK ZTRCO,ZTRSL
C     EXTERNAL ZTRXX,ZMACH
...
C
      COMPLEX*16 A(15,15),B(15),BT(15),X(15),XEXACT(15),XT(15),Z(15)
      COMPLEX*16 AINV(15,15),DET(2)
      COMPLEX*16 ZDOTC,STUFF,T
      DOUBLE PRECISION ANORM,AINORM,RCOND,COND,COND1,ZMACH,EPS
      DOUBLE PRECISION ENORM,ETNORM,RNORM,RTNORM,XNORM,XTNORM,EN,DZASUM
...
C
         ...
         DO 120 J = 1, N
            DO 110 I = 1, N
               IF (CABS1(A(I,J)) .EQ. 0.0D0) GO TO 100
...
C
  ...
  180             CONTINUE
  190          CONTINUE
               CALL ZTRDI(AINV,LDA,N,DET,110+JOB,INFO)
               AINORM = 0.0D0
               DO 200 J = 1, N
                  IF (JOB .EQ. 0)
     *               AINORM = DMAX1(AINORM,DZASUM(N-J+1,AINV(J,J),1))
                  IF (JOB .EQ. 1)
     *               AINORM = DMAX1(AINORM,DZASUM(J,AINV(1,J),1))
  200          CONTINUE
               COND1 = ANORM*AINORM
...
C
               CALL ZTRSL(A,LDA,N,X,JOB,INFO)
               CALL ZTRSL(A,LDA,N,XT,JOB+10,INFO)
C
...
C
               DO 260 J = 1, N
                  DO 250 I = 1, N
                     IF (CABS1(A(I,J)-STUFF) .NE. 0.0D0) GO TO 240
                        A(I,J) = (0.0D0,0.0D0)
                        AINV(I,J) = (0.0D0,0.0D0)
  240                CONTINUE
  250             CONTINUE
  260          CONTINUE
C
...
C
               ...
               DO 270 J = 1, N
                  ...
                  T = -X(J)
                  CALL ZAXPY(N,T,A(1,J),1,B,1)
                  BT(J) = BT(J) - ZDOTC(N,A(1,J),1,XT,1)
  270          CONTINUE
               RNORM = DZASUM(N,B,1)
...
C
      ...
      RETURN
C
C     MOST FORMATS, ALSO SOME IN ZTRXX
C
  370 FORMAT (1H )
  380 FORMAT (22H1LINPACK TESTER, ZTR**)
  ...
  730 FORMAT (29H THIS VERSION DATED 08/14/78.)
      END
      SUBROUTINE ZTRXX(A,LDA,N,KASE,LUNIT)
      INTEGER LDA,N,KASE,LUNIT
      COMPLEX*16 A(LDA,1)
C
C     GENERATES COMPLEX*16 TRIANGULAR TEST MATRICES
C
C     EXTERNAL ZMACH
C     FORTRAN DCMPLX,DBLE,FLOAT
      COMPLEX*16 T1,T2
...
C
  ...
  120    FORMAT (5H KASE, I3, 3X, 16HHALF OF RANK ONE / 4H N =, I4)
         DO 140 I = 1, N
            DO 130 J = 1, N
               A(I,J) = (0.0D0,0.0D0)
               IF (I .GE. J) A(I,J) = DCMPLX(10.0D0**(I-J),0.0D0)
  130       CONTINUE
  140    CONTINUE
      GO TO 470
C
...
C
  ...
  210    FORMAT (5H KASE, I3, 3X, 16HTEST COND        / 4H N =, I4)
         DO 230 I = 1, N
            DO 220 J = 1, N
               IF (I .EQ. J) A(I,J) = (1.0D0,0.0D0)
               IF (I .GT. J) A(I,J) = (0.0D0,0.0D0)
               IF (I .LT. J) A(I,J) = (-1.0D0,0.0D0)
  220       CONTINUE
  230    CONTINUE
      GO TO 470
C
...
C
  ...
  290    FORMAT (5H KASE, I3, 3X, 16HUPPER TRIANGULAR / 4H N =, I4)
         DO 310 I = 1, N
            DO 300 J = 1, N
               IF (I .GT. J) A(I,J) = (0.0D0,0.0D0)
               IF (I .LE. J) A(I,J) = DBLE(FLOAT(I-J-1))*(1.0D0,1.0D0)
  300       CONTINUE
  310    CONTINUE
      GO TO 470
C
  ...
  330    FORMAT (5H KASE, I3, 3X, 16HLOWER TRIANGULAR / 4H N =, I4)
         DO 350 I = 1, N
            DO 340 J = 1, N
               ...
               IF (I .GE. J) A(I,J) = DBLE(FLOAT(I-J+1))*(1.0D0,-1.0D0)
  340       CONTINUE
  350    CONTINUE
      GO TO 470
C
  ...
  380    FORMAT (14H TINY        =, 1PD13.5)
         DO 400 I = 1, N
            DO 390 J = 1, N
               ...
               IF (I .LE. J)
     *            A(I,J) = TINY
     *                     *DCMPLX(DBLE(FLOAT(J))/DBLE(FLOAT(I)),1.0D0)
  390       CONTINUE
  400    CONTINUE
      GO TO 470
C
  ...
  430    FORMAT (14H HUGE        =, 1PD16.5)
         DO 450 I = 1, N
            DO 440 J = 1, N
               ...
               IF (I .GE. J)
     *            A(I,J) = HUGE
     *                     *DCMPLX(DBLE(FLOAT(J))/DBLE(FLOAT(I)),0.0D0)
  440       CONTINUE
  450    CONTINUE
      GO TO 470
C
