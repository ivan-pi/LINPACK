...
C
C     LINPACK CQRDC,CQRSL
C     EXTERNAL CQRBM,CQRFM,CQRRX,CXGEN,CMACH
...
C
      ...
      COMPLEX BETA(25),QRAUX(25),QY(25),RSD(25),S(25),WORK(25)
      COMPLEX X(25,25),XBETA(25),XX(25,25),Y(25),Y1(25),Y2(25)
      REAL CMACH,TINY,HUGE
      CHARACTER CONT
      LOGICAL NOTWRT
      DATA CONT /' '/
      LDX = 25
...
C
            DO 60 I = 1, N
               Y1(I) = (0.0E0,0.0E0)
               Y2(I) = (2.0E0,0.0E0)*TINY
               IF (I .EQ. P + 1)
     *            Y2(I) = Y2(I) - CMPLX(FLOAT(N),0.0E0)*TINY
               DO 50 J = 1, P
                  X(I,J) = X(I,J)*TINY
                  Y1(I) = Y1(I) + X(I,J)
                  XX(I,J) = X(I,J)
...
C           REDUCE THE MATRIX.
C
            CALL CQRDC(X,LDX,N,P,QRAUX,JPVT,WORK,0)
            IF (NOTWRT) GO TO 70
               WRITE (LUNIT,610)
               CALL CARRAY(X,LDX,N,P,4,LUNIT)
               WRITE (LUNIT,620)
               CALL CARRAY(QRAUX,P,P,1,-4,LUNIT)
   70       CONTINUE
C
C           COMPUTE THE STATISTICS FOR THE FOWARD AND BACK
C           MULTIPLICATIONS.
C
            CALL CQRFM(X,LDX,N,P,XX,WORK,QRAUX,FSTAT)
            CALL CQRBM(X,LDX,N,P,XX,WORK,QRAUX,BSTAT)
C
C           SOLVE THE LEAST SQUARES PROBLEM.
C
            CALL CQRSL(X,LDX,N,P,QRAUX,Y,QY,RSD,BETA,RSD,XBETA,111,
     *                 INFO)
C
...
C
            ...
            XBMAX = 0.0E0
            DO 80 I = 1, N
               RSTAT = AMAX1(RSTAT,CABS(Y2(I)-RSD(I)))
               RMAX = AMAX1(RMAX,CABS(Y2(I)))
               XBSTAT = AMAX1(XBSTAT,CABS(Y1(I)-XBETA(I)))
               XBMAX = AMAX1(XBMAX,CABS(Y2(I)))
   80       CONTINUE
            RSTAT = RSTAT/(CMACH(1)*RMAX)
            XBSTAT = XBSTAT/(CMACH(1)*XBMAX)
            BESTAT = 0.0E0
            DO 90 J = 1, P
               BESTAT = AMAX1(BESTAT,CABS((1.0E0,0.0E0)-BETA(J)))
   90       CONTINUE
            BESTAT = BESTAT/CMACH(1)
...
C
  ...
  120       CONTINUE
            ...
            IF (NOTWRT) GO TO 130
               WRITE (LUNIT,610)
               CALL CARRAY(X,LDX,N,P,10,LUNIT)
  130       CONTINUE
            DO 150 I = 1, N
               DO 140 J = 1, P
                  XX(I,J) = X(I,J)
  ...
  150       CONTINUE
            ...
            IF (NOTWRT) GO TO 160
               ...
               CALL CARRAY(X,LDX,N,P,10,LUNIT)
               WRITE (LUNIT,620)
               CALL CARRAY(QRAUX,N,N,1,-4,LUNIT)
  160       CONTINUE
            CALL CQRFM(X,LDX,N,P,XX,WORK,QRAUX,FSTAT)
            CALL CQRBM(X,LDX,N,P,XX,WORK,QRAUX,BSTAT)
...
C
  ...
  220       CONTINUE
            ...
            JPVT(4) = -1
            JPVT(7) = -1
            CALL CQRDC(X,LDX,N,P,QRAUX,JPVT,WORK,1)
            IF (NOTWRT) GO TO 230
               WRITE (LUNIT,610)
               CALL CARRAY(X,LDX,N,P,9,LUNIT)
               WRITE (LUNIT,620)
               CALL CARRAY(QRAUX,P,P,1,-9,LUNIT)
  230       CONTINUE
            WRITE (LUNIT,630) (JPVT(J), J = 1, P)
            CALL CQRRX(XX,LDX,N,P,JPVT)
            CALL CQRFM(X,LDX,N,P,XX,WORK,QRAUX,FSTAT)
            CALL CQRBM(X,LDX,N,P,XX,WORK,QRAUX,BSTAT)
            WRITE (LUNIT,570) FSTAT,BSTAT
         GO TO 540
...
C
  ...
  280       CONTINUE
            ...
            IF (NOTWRT) GO TO 290
               ...
               CALL CARRAY(X,LDX,N,P,10,LUNIT)
               WRITE (LUNIT,620)
               CALL CARRAY(QRAUX,P,P,1,-10,LUNIT)
  290       CONTINUE
            WRITE (LUNIT,630) (JPVT(J), J = 1, P)
            CALL CQRRX(XX,LDX,N,P,JPVT)
            CALL CQRFM(X,LDX,N,P,XX,WORK,QRAUX,FSTAT)
...
C
  ...
  310       CONTINUE
            ...
            IF (NOTWRT) GO TO 320
               ...
               CALL CARRAY(X,LDX,N,P,1,LUNIT)
               WRITE (LUNIT,620)
               CALL CARRAY(QRAUX,P,P,1,-1,LUNIT)
  320       CONTINUE
            CALL CQRFM(X,LDX,N,P,XX,WORK,QRAUX,FSTAT)
            CALL CQRBM(X,LDX,N,P,XX,WORK,QRAUX,BSTAT)
            WRITE (LUNIT,570) FSTAT,BSTAT
...
C
  ...
  390       CONTINUE
            DO 400 I = 1, N
               Y1(I) = (0.0E0,0.0E0)
               Y2(I) = (2.0E0,0.0E0)
               IF (I .EQ. P + 1) Y2(I) = Y2(I) - CMPLX(FLOAT(N),0.0E0)
               Y1(I) = Y1(I) + X(I,1)
               Y(I) = Y1(I) + Y2(I)
               XX(I,1) = X(I,1)
  400       CONTINUE
...
C
  ...
  450       CONTINUE
            ...
            IF (NOTWRT) GO TO 460
               ...
               CALL CARRAY(X,LDX,N,P,10,LUNIT)
               WRITE (LUNIT,620)
               CALL CARRAY(QRAUX,N,N,1,-1,LUNIT)
  460       CONTINUE
            WRITE (LUNIT,630) (JPVT(J), J = 1, P)
            CALL CQRRX(XX,LDX,N,P,JPVT)
            CALL CQRFM(X,LDX,N,P,XX,WORK,QRAUX,FSTAT)
  ...
  470    CONTINUE
            ...
            DO 490 I = 1, N
               DO 480 JJJ = 1, 3
                  ...
                  JJ = 3*J
                  X(I,JJ) = X(I,J)
                  X(I,JJ-1) = X(I,JJ)/(2.0E0,0.0E0)
                  X(I,JJ-2) = X(I,JJ-1)/(2.0E0,0.0E0)
  480          CONTINUE
  490       CONTINUE
            IF (NOTWRT) GO TO 500
               WRITE (LUNIT,610)
...
C
  ...
  600 FORMAT (22H1LINPACK TESTER, CQR** /
...
C
   ...
   70 FORMAT (1H , 8E13.6)
      END
      SUBROUTINE CQRBM(X,LDX,N,P,XX,QR,QRAUX,BSTAT)
      INTEGER LDX,N,P
      REAL BSTAT
      COMPLEX X(LDX,1),XX(LDX,1),QR(1),QRAUX(1)
C
C     CQRBM TAKES THE OUTPUT OF CQRDC AND MULTIPLIES THE FACTORS
C     Q AND R TOGETHER.  THE RESULTS ARE COMPARED WITH XX,
C     WHICH PRESUMABLY CONTAINS THE ORIGINAL MATRIX.
C
C     LINPACK CQRSL
C     EXTERNAL CMACH
C     FORTRAN AMAX1,CABS,MIN0
C
   ...
   20    CONTINUE
   30    CONTINUE
         CALL CQRSL(X,LDX,N,P,QRAUX,QR,QR,QR,QR,QR,QR,10000,INFO)
         DO 40 I = 1, N
            EMAX = AMAX1(EMAX,CABS(XX(I,J)-QR(I)))
   ...
   70    CONTINUE
      GO TO 90
   80 CONTINUE
         BSTAT = (EMAX/XMAX)/CMACH(1)
   90 CONTINUE
      RETURN
      END
      SUBROUTINE CQRFM(X,LDX,N,P,XX,QTX,QRAUX,FSTAT)
      INTEGER LDX,N,P
      REAL FSTAT
      COMPLEX X(LDX,1),XX(LDX,1),QTX(1),QRAUX(1)
C
C     CQRFM TAKES THE OUTPUT OF CQRDC AND APPLIES THE
...
C
      ...
      DO 50 J = 1, P
         DO 10 I = 1, N
            QTX(I) = XX(I,J)
   10    CONTINUE
         CALL CQRSL(X,LDX,N,P,QRAUX,QTX,QTX,QTX,QTX,QTX,QTX,1000,INFO)
         IU = MIN0(J,N)
         DO 20 I = 1, IU
            EMAX = AMAX1(EMAX,CABS(X(I,J)-QTX(I)))
            RMAX = AMAX1(RMAX,CABS(X(I,J)))
   20    CONTINUE
         JP1 = J + 1
         IF (N .LT. JP1) GO TO 40
         DO 30 I = JP1, N
            EMAX = AMAX1(EMAX,CABS(QTX(I)))
   30    CONTINUE
   40    CONTINUE
   50 CONTINUE
   ...
   70    CONTINUE
      GO TO 90
   80 CONTINUE
         FSTAT = (EMAX/RMAX)/CMACH(1)
   90 CONTINUE
      RETURN
      END
      SUBROUTINE CQRRX(XX,LDX,N,P,JP)
      INTEGER N,P,LDX
      INTEGER JP(1)
      COMPLEX XX(LDX,1)
C
C     CQRRX REARRANGES THE COLUMNS OF XX IN THE ORDER SPECIFIED
C     BY JPVT.
C
C     EXTERNAL CSWAP
C
...
C           ...EXIT
   ...
   20       CONTINUE
   30       CONTINUE
            CALL CSWAP(N,XX(1,I),1,XX(1,J),1)
            DO 40 K = I1, P
   ...
   60 CONTINUE
      ...
      COMPLEX X(LDX,1),S(1)
C
C     CXGEN GENERATES AN N X P MATRIX X HAVING SINGULAR
C     VALUES S(1), S(2), ..., S(M), WHERE M = MIN(N,P)
C
C     FORTRAN CMPLX,COS,FLOAT,MIN0,SIN
C
      INTEGER I,J,M,MP1
      REAL FN,FP
      COMPLEX FAC,RU,T
