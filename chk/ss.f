...
C
  ...
  380             CONTINUE
                  IF (KSTEP .EQ. 2)
     *               CALL SSWAP(1,A(KS,K+1),1,A(K,K+1),1)
  390          CONTINUE
               K = K + KSTEP
            GO TO 310
  400       CONTINUE
...
C
      ...
      RETURN
C
C     MOST FORMATS, ALSO SOME IN SSIXX
C
  530 FORMAT (1H )
  540 FORMAT (29H1LINPACK TESTER, SSI**, SSP**)
  ...
  950 FORMAT (29H THIS VERSION DATED 08/14/78.)
      END
      SUBROUTINE SSIXX(A,LDA,N,KASE,LUNIT)
      INTEGER LDA,N,KASE,LUNIT
      REAL A(LDA,1)
C
C     GENERATES REAL SYMMETRIC INDEFINITE TEST MATRICES
C
C     EXTERNAL SMACH
C     FORTRAN ABS,FLOAT,IABS
      REAL T
...
C
   ...
   20    FORMAT (5H KASE, I3, 3X, 13HREAL HILBERT  / 4H N =, I4)
         T = 1.0E0
         T = SIGN(1.0E0,T)
         DO 40 J = 1, N
            DO 30 I = 1, J
...
C
   ...
   80    FORMAT (5H KASE, I3, 3X, 16HTRIDIAGONAL      / 4H N =, I4)
         T = 1.0E0
         IF (KASE .EQ. 7) T = 3.0E0
         IF (KASE .EQ. 8) T = 100.0E0
         DO 100 J = 1, N
            DO 90 I = 1, J
               A(I,J) = 0.0E0
  ...
  340    FORMAT (5H KASE, I3, 3X, 16H                 / 4H N =, I4)
         DO 370 J = 1, N
            DO 360 I = 1, J
               ...
               A(J,I) = A(I,J)
               IF (I .NE. J) GO TO 350
                  IF (KASE .EQ. 15) A(I,I) = 26.0E0
                  IF (KASE .EQ. 16) A(I,I) = FLOAT(I)
  350          CONTINUE
  360       CONTINUE
  370    CONTINUE
      GO TO 490
...
C
  ...
  400    FORMAT (14H TINY        =, 1PE13.5)
         DO 420 J = 1, N
            DO 410 I = 1, J
               A(I,J) = TINY*(FLOAT(IABS(I-J))/FLOAT(I+J))
               A(J,I) = A(I,J)
  410       CONTINUE
  420    CONTINUE
      GO TO 490
...
C
  ...
  450    FORMAT (14H HUGE        =, 1PE13.5)
         DO 470 J = 1, N
            DO 460 I = 1, J
               A(I,J) = HUGE*(FLOAT(IABS(I-J))/FLOAT(I+J))
               A(J,I) = A(I,J)
  460       CONTINUE
  470    CONTINUE
      GO TO 490
