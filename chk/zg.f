...
C
      ...
      STOP
      END
      SUBROUTINE ZGETS(LUNIT)
C     LUNIT IS THE OUTPUT UNIT NUMBER
C
C     TESTS
C        ZGECO,ZGEFA,ZGESL,ZGEDI,ZGBCO,ZGBFA,ZGBSL,ZGBDI
C
...
C
C     LINPACK ZGECO,ZGESL,ZGEDI,ZGBCO,ZGBSL,ZGBDI
C     EXTERNAL ZGEXX,ZMACH
C     BLAS ZAXPY,ZDOTC,ZSCAL,DZASUM
C     FORTRAN DABS,DMAX1,DCONJG,DBLE,FLOAT
C     FORTRAN MAX0,MIN0
C
C     INTERNAL VARIABLES
C
      COMPLEX*16 A(15,15),AB(43,15),AINV(15,15),ASAVE(15,15)
      COMPLEX*16 B(15),BT(15),ZDOTC,DET(2),DETB(2)
      COMPLEX*16 X(15),XB(15),XEXACT(15),XT(15),XTB(15),T,Z(15)
      DOUBLE PRECISION AINORM,ANORM,ZMACH,COND,COND1,EN,ENORM,EPS
      DOUBLE PRECISION ETNORM,FNI,FNORM,ONEPX,RCOND,RCONDB,RNORM
      DOUBLE PRECISION RTNORM,Q(8),QS(8),DZASUM,XNORM,XTNORM
...
C
         DO 90 I = 1, N
            ...
            DO 80 J = 1, N
               ...
               BT(I) = BT(I) + DCONJG(A(J,I))*XEXACT(J)
   80       CONTINUE
            X(I) = B(I)
            XT(I) = BT(I)
            XB(I) = X(I)
...
C
         ...
         DO 140 J = 1, N
            DO 130 I = 1, N
               IF (CABS1(ASAVE(I,J)) .EQ. 0.0D0) GO TO 120
  ...
  170       CONTINUE
C
            CALL ZGBCO(AB,LDAB,N,ML,MU,IPVTB,RCONDB,Z)
C
  ...
  180       CONTINUE
            KOUNT = 0
            DO 190 J = 1, N
               IF (CABS1(AB(M,J)-A(J,J)) .NE. 0.0D0) KOUNT = KOUNT + 1
...
C
  ...
  210       CONTINUE
               ...
               ONEPX = 1.0D0 + RCOND
               IF (ONEPX .EQ. 1.0D0) WRITE (LUNIT,490)
C
...
C
  ...
  220             CONTINUE
  230          CONTINUE
               CALL ZGEDI(AINV,LDA,N,IPVT,DET,Z,11)
               AINORM = 0.0D0
               DO 240 J = 1, N
...
C
C              SOLVE  A*X = B  AND  CTRANS(A)*XT = BT
C
               CALL ZGESL(A,LDA,N,IPVT,X,0)
               CALL ZGESL(A,LDA,N,IPVT,XT,1)
C
...
C              MORE BAND COMPARE
C
               CALL ZGBSL(AB,LDAB,N,ML,MU,IPVTB,XB,0)
               CALL ZGBSL(AB,LDAB,N,ML,MU,IPVTB,XTB,1)
               KOUNT = 0
               DO 280 I = 1, N
                  IF (CABS1(XB(I)-X(I)) .NE. 0.0D0) KOUNT = KOUNT + 1
                  IF (CABS1(XTB(I)-XT(I)) .NE. 0.0D0) KOUNT = KOUNT + 1
  280          CONTINUE
               IF (KOUNT .EQ. 0) GO TO 290
                  ...
                  KBF = .TRUE.
  290          CONTINUE
               CALL ZGBDI(AB,LDAB,N,ML,MU,IPVTB,DETB)
               IF (CABS1(DETB(1)-DET(1)) .EQ. 0.0D0
     *             .AND. CABS1(DETB(2)-DET(2)) .EQ. 0.0D0) GO TO 300
                  WRITE (LUNIT,780)
...
C
               XNORM = DZASUM(N,X,1)
               XTNORM = DZASUM(N,XT,1)
               ENORM = 0.0D0
               ETNORM = 0.0D0
               FNORM = 0.0D0
               DO 350 J = 1, N
                  ENORM = ENORM + CABS1(X(J)-XEXACT(J))
                  ETNORM = ETNORM + CABS1(XT(J)-XEXACT(J))
                  T = -X(J)
                  CALL ZAXPY(N,T,ASAVE(1,J),1,B,1)
                  BT(J) = BT(J) - ZDOTC(N,ASAVE(1,J),1,XT,1)
                  FNI = 0.0D0
                  DO 340 I = 1, N
                     FNI = FNI + CABS1(ASAVE(I,J)-A(I,J))
  340             CONTINUE
                  IF (FNI .GT. FNORM) FNORM = FNI
  350          CONTINUE
               RNORM = DZASUM(N,B,1)
               RTNORM = DZASUM(N,BT,1)
C
C              A*INV(A) - I
C
               AINORM = 0.0D0
...
C
      ...
      RETURN
C
C     MOST FORMATS, ALSO SOME IN ZGEXX
C
  450 FORMAT (1H )
  460 FORMAT (29H1LINPACK TESTER, ZGE**, ZGB**)
  470 FORMAT ( / 14H EPSILON     =, 1PD13.5)
  480 FORMAT ( / 19H EXACT SINGULARITY. /)
  490 FORMAT ( / 16H MAYBE SINGULAR. /)
  500 FORMAT (14H COND        =, 1PD13.5)
  510 FORMAT (14H ACTUAL COND =, 1PD13.5)
  520 FORMAT ( / 4H X =)
  530 FORMAT ( / 5H XT =)
  540 FORMAT (14H ERROR NORMS =, 1P2D13.5)
  550 FORMAT (14H RESID NORMS =, 1P2D13.5)
  ...
  880 FORMAT (29H THIS VERSION DATED 08/14/78.)
      END
      SUBROUTINE ZGEXX(A,LDA,N,KASE,LUNIT)
      INTEGER LDA,N,KASE,LUNIT
      COMPLEX*16 A(LDA,1)
C
C     GENERATES COMPLEX*16 GENERAL TEST MATRICES
C
C     EXTERNAL ZMACH
C     FORTRAN DCMPLX,DBLE,FLOAT,MAX0
      COMPLEX*16 T1,T2
      DOUBLE PRECISION ZMACH,HUGE,TINY
      INTEGER I,J
      DOUBLE PRECISION DREAL,DIMAG
      COMPLEX*16 ZDUMR,ZDUMI
      DREAL(ZDUMR) = ZDUMR
...
C
   ...
   70    FORMAT (5H KASE, I3, 3X, 16HMONOELEMENTAL    / 4H N =, I4)
         IF (KASE .EQ. 4) A(1,1) = (3.0D0,1.0D0)
...
C
   ...
   90    FORMAT (5H KASE, I3, 3X, 16HTRIDIAGONAL      / 4H N =, I4)
         T1 = (1.0D0,0.0D0)
         T2 = (1.0D0,0.0D0)
         IF (KASE .EQ. 7) T1 = (100.0D0,100.0D0)
         IF (KASE .EQ. 8) T2 = (100.0D0,100.0D0)
         DO 110 I = 1, N
            DO 100 J = 1, N
               A(I,J) = (0.0D0,0.0D0)
...
C
  ...
  170    FORMAT (5H KASE, I3, 3X, 16HZERO COLUMN      / 4H N =, I4)
         DO 190 I = 1, N
            DO 180 J = 1, N
               T1 = DCMPLX(DBLE(FLOAT(J-3)),0.0D0)
               T2 = DCMPLX(DBLE(FLOAT(I)),0.0D0)
               A(I,J) = T1/T2
  180       CONTINUE
  190    CONTINUE
      GO TO 470
...
C
  ...
  210    FORMAT (5H KASE, I3, 3X, 16HTEST COND        / 4H N =, I4)
         DO 230 I = 1, N
            DO 220 J = 1, N
               IF (I .EQ. J) A(I,J) = DCMPLX(DBLE(FLOAT(I)),0.0D0)
               IF (I .GT. J) A(I,J) = DCMPLX(DBLE(FLOAT(J-2)),0.0D0)
               IF (I .LT. J) A(I,J) = DCMPLX(DBLE(FLOAT(I-2)),0.0D0)
  220       CONTINUE
  230    CONTINUE
      GO TO 470
C
...
C
  ...
  250    FORMAT (5H KASE, I3, 3X, 16HIDENTITY         / 4H N =, I4)
         DO 270 I = 1, N
            DO 260 J = 1, N
               IF (I .EQ. J) A(I,I) = (1.0D0,0.0D0)
               IF (I .NE. J) A(I,J) = (0.0D0,0.0D0)
  260       CONTINUE
  270    CONTINUE
      GO TO 470
C
...
C
  ...
  290    FORMAT (5H KASE, I3, 3X, 16HUPPER TRIANGULAR / 4H N =, I4)
         DO 310 I = 1, N
            DO 300 J = 1, N
               IF (I .GT. J) A(I,J) = (0.0D0,0.0D0)
               IF (I .LE. J)
     *            A(I,J) = DCMPLX(DBLE(FLOAT(J-I+1)),DBLE(FLOAT(J-I)))
  300       CONTINUE
  310    CONTINUE
      GO TO 470
C
...
C
  ...
  330    FORMAT (5H KASE, I3, 3X, 16HLOWER TRIANGULAR / 4H N =, I4)
         DO 350 I = 1, N
            DO 340 J = 1, N
               IF (I .LT. J) A(I,J) = (0.0D0,0.0D0)
               IF (I .GE. J) A(I,J) = DCMPLX(DBLE(FLOAT(I-J+1)),-1.0D0)
  340       CONTINUE
  350    CONTINUE
      GO TO 470
C
...
C
  ...
  380    FORMAT (14H TINY        =, 1PD13.5)
         DO 400 I = 1, N
            DO 390 J = 1, N
               A(I,J) = DCMPLX(TINY*DBLE(FLOAT(J))
     *                         /DBLE(FLOAT(MAX0(I,J))),0.0D0)
  390       CONTINUE
  400    CONTINUE
      GO TO 470
C
...
C
  ...
  430    FORMAT (14H HUGE        =, 1PD13.5)
         DO 450 I = 1, N
            DO 440 J = 1, N
               A(I,J) = DCMPLX(HUGE*DBLE(FLOAT(J))
     *                         /DBLE(FLOAT(MAX0(I,J))),0.0D0)
  440       CONTINUE
  450    CONTINUE
      GO TO 470
