...
C
      ...
      STOP
      END
      SUBROUTINE DSITS(LUNIT)
C     LUNIT IS THE OUTPUT UNIT NUMBER
C
C     TESTS
C        DSICO,DSIFA,DSISL,DSIDI,DSPCO,DSPFA,DSPSL,DSPDI
C
...
C
C     LINPACK DSICO,DSISL,DSIDI,DSPCO,DSPSL,DSPDI
C     EXTERNAL DSIXX,DMACH
C     BLAS DAXPY,DSWAP,DASUM,DCOPY
C     FORTRAN DABS,DMAX1,DBLE,FLOAT,IABS
C
C     INTERNAL VARIABLES
C
      DOUBLE PRECISION A(15,15),AINV(15,15),ASAVE(15,15),AJK,AJKP1
      DOUBLE PRECISION AP(120),B(15),X(15),XEXACT(15)
      DOUBLE PRECISION XP(15),T,Z(15)
      DOUBLE PRECISION ANORM,AINORM,COND,COND1,DET(2),DETP(2)
...
C
         RCOND = -1.0D0
         CALL DSICO(A,LDA,N,KPVT,RCOND,Z)
...
C
  ...
  130    CONTINUE
         RCONDP = -1.0D0
         CALL DSPCO(AP,N,KPVTP,RCONDP,Z)
...
C
         IF (RCOND .GT. 0.0D0) GO TO 180
            ...
            KSING = KSING + 1
            CALL DSIDI(A,LDA,N,KPVT,DET,INERT,Z,110)
...
C
  ...
  190          CONTINUE
  200       CONTINUE
            CALL DSIDI(AINV,LDA,N,KPVT,DET,INERT,Z,111)
            AINORM = 0.0D0
            DO 220 J = 1, N
               DO 210 I = J, N
                  AINV(I,J) = AINV(J,I)
...
C
  ...
  250       CONTINUE
            IF (KOUNT .EQ. 0) GO TO 260
               ...
               KPF = .TRUE.
  260       CONTINUE
            CALL DSPDI(AP,N,KPVTP,DETP,INERTP,Z,111)
...
C
  ...
  380             CONTINUE
                  IF (KSTEP .EQ. 2)
     *               CALL DSWAP(1,A(KS,K+1),1,A(K,K+1),1)
  390          CONTINUE
               K = K + KSTEP
            GO TO 310
  400       CONTINUE
...
C
  ...
  490          CONTINUE
  500       CONTINUE
            IF (KOUNT .EQ. 0) WRITE (LUNIT,930)
            IF (KOUNT .NE. 0) WRITE (LUNIT,940) (IQ(I), I = 1, 6)
            WRITE (LUNIT,530)
  510    CONTINUE
C
         IF (.NOT.KPF) WRITE (LUNIT,830)
...
C
      ...
      WRITE (LUNIT,860) KSING
      WRITE (LUNIT,850) KPFAIL
      WRITE (LUNIT,660) KSUSP
      WRITE (LUNIT,870)
      RETURN
C
C     MOST FORMATS, ALSO SOME IN DSIXX
C
  530 FORMAT (1H )
  540 FORMAT (29H1LINPACK TESTER, DSI**, DSP**)
  ...
  770 FORMAT (2G14.6)
  780 FORMAT (2G14.6)
  790 FORMAT (14H DET FRACT   =, F9.5)
  800 FORMAT (14H DET EXPON   =, F9.0)
  810 FORMAT (14H INERTIA     =, 3I3 / 14X, 9H  +  -  0)
  820 FORMAT (14H KPVT        =, 15I3)
  830 FORMAT ( / 22H PACKED ROUTINES AGREE)
  840 FORMAT ( / 30H PACKED ROUTINES DO NOT AGREE,)
  850 FORMAT (28H NUMBER OF PACKED FAILURES =, I4)
  860 FORMAT (23H NUMBER OF ZERO PIVOT =, I4)
  870 FORMAT ( / 12H END OF TEST)
  880 FORMAT (8H RCOND =, 1P2D13.5)
  890 FORMAT (12H KOUNT(FA) =, I4)
  900 FORMAT (12H KOUNT(SL) =, I4)
  910 FORMAT (8H DET   =, F9.5, F9.0)
  920 FORMAT (12H KOUNT(DI) =, I4)
  930 FORMAT (21H NO SUSPICIOUS RATIOS)
  940 FORMAT (I8, 5I10 / 7X, 28H1 INDICATES SUSPICIOUS RATIO)
  950 FORMAT (29H THIS VERSION DATED 08/14/78.)
      END
      SUBROUTINE DSIXX(A,LDA,N,KASE,LUNIT)
      INTEGER LDA,N,KASE,LUNIT
      DOUBLE PRECISION A(LDA,1)
C
C     GENERATES DOUBLE PRECISION SYMMETRIC INDEFINITE TEST MATRICES
C
C     EXTERNAL DMACH
C     FORTRAN DABS,DBLE,FLOAT,IABS
      DOUBLE PRECISION T
...
C
   ...
   20    FORMAT (5H KASE, I3, 3X, 25HDOUBLE PRECISION HILBERT  /
     *           4H N =, I4)
         T = 1.0D0
         T = DSIGN(1.0D0,T)
         DO 40 J = 1, N
...
C
   ...
   80    FORMAT (5H KASE, I3, 3X, 16HTRIDIAGONAL      / 4H N =, I4)
         T = 1.0D0
         IF (KASE .EQ. 7) T = 3.0D0
         IF (KASE .EQ. 8) T = 100.0D0
         DO 100 J = 1, N
            DO 90 I = 1, J
               A(I,J) = 0.0D0
...
C
  ...
  300    FORMAT (5H KASE, I3, 3X, 16HINERTIA = 1,1,1  / 4H N =, I4)
         DO 320 I = 1, N
            DO 310 J = 1, N
               A(I,J) = 0.0D0
  310       CONTINUE
  ...
  340    FORMAT (5H KASE, I3, 3X, 16H                 / 4H N =, I4)
         DO 370 J = 1, N
            DO 360 I = 1, J
               ...
               A(J,I) = A(I,J)
               IF (I .NE. J) GO TO 350
                  IF (KASE .EQ. 15) A(I,I) = 26.0D0
                  IF (KASE .EQ. 16) A(I,I) = DBLE(FLOAT(I))
  350          CONTINUE
  360       CONTINUE
  370    CONTINUE
      GO TO 490
...
C
  ...
  400    FORMAT (14H TINY        =, 1PD13.5)
         DO 420 J = 1, N
            DO 410 I = 1, J
               A(I,J) = TINY*(DBLE(FLOAT(IABS(I-J)))/DBLE(FLOAT(I+J)))
               A(J,I) = A(I,J)
  410       CONTINUE
  420    CONTINUE
      GO TO 490
...
C
  ...
  450    FORMAT (14H HUGE        =, 1PD13.5)
         DO 470 J = 1, N
            DO 460 I = 1, J
               A(I,J) = HUGE*(DBLE(FLOAT(IABS(I-J)))/DBLE(FLOAT(I+J)))
               A(J,I) = A(I,J)
  460       CONTINUE
  470    CONTINUE
      GO TO 490
